...
# Generate HTML Quiz QR
qr = qrcode.QRCode(version=1, box_size=10, border=1)
url = f"https://tararaidolls.github.io/romanian/one-verb-a-day/quiz_part1.html"
qr.add_data(url)
qr.make(fit=True)
img = qr.make_image(fill_color="black", back_color="white")
img.save(f"/workspace/one-verb-a-day/romanian/qr/quiz_1.png")
...
class Vol1Book(FPDF):
    ...
    def draw_verb_page(self, day_num, verb_info):
        ...
        self.image(qr_path, x=qr_x, y=self.get_y(), w=qr_w)
        
        self.set_y(self.get_y() + qr_w + 2)
        self.set_font('DS', '', 7)
    ...
    def draw_quiz_page(self, part_num):
        self.add_page()
        self.set_y(100)
        self.set_font('DS', 'B', 32)
        self.set_text_color(*BLACK)
        self.cell(0, 20, f"PART {part_num} REVIEW", 0, 1, 'C')
        
        self.ln(20)
        self.set_font('DS', '', 14)
        self.set_text_color(80, 80, 80)
        self.cell(0, 10, "You have completed 25 verbs. Are you ready to test your knowledge?", 0, 1, 'C')
        self.ln(20)
        
        qr_w = 40
        qr_x = MARGIN + (CONTENT_W - qr_w) / 2
        qr_path = f"/workspace/one-verb-a-day/romanian/qr/quiz_{part_num}.png"
        self.image(qr_path, x=qr_x, y=self.get_y(), w=qr_w)
        
        self.set_y(self.get_y() + qr_w + 5)
        self.set_font('DS', 'B', 12)
        self.set_text_color(*BLACK)
        self.cell(0, 10, "Scan to take the interactive 25-verb Quiz!", 0, 1, 'C')
        self.set_font('DS', '', 10)
        self.set_text_color(100, 100, 100)
        self.cell(0, 8, "Self-correcting flashcards & multiple choice.", 0, 1, 'C')

pdf = Vol1Book()
pdf.draw_verb_page(1, verbs_100[0])
pdf.draw_verb_page(2, verbs_100[1])
pdf.draw_quiz_page(1)

pdf.output("/workspace/one-verb-a-day/test_qr_book1.pdf")
